...
if [[ "$ID" == "debian" || "$ID" == "ubuntu" \
    ...
    if $EXTRA_DEPENDENCIES; then
        ...
        sudo apt-get install -y -qq python3-pip ninja-build meson
    fi
    sudo apt-get install -y -qq \
        wget \
    ...
    if [[ $(echo "$VERSION_ID>=23.04" |bc -l) -eq "1" &&  "$ID" == "ubuntu" ]]; then
        ...
        echo "Your ubuntu version does not have the gtk4 libraries. You can compile Inkscape versions up to 1.3.x branch, but 1.4 and more need at least ubuntu 23.10"
    fi
