...
SvgBuilder::ConvertedColor SvgBuilder::convertGfxColor(const GfxColor *color, GfxColorSpace *space)
{
    std::string icc = "";
    ...
    switch (space->getMode()) {
        ...
        case csSeparation:
            mode = "csSeparation";
            orgColor = ((GfxSeparationColorSpace*)space)->getName()->toStr();
            break;
        case csDeviceN:
            mode = "csDeviceN";
            break;
    ...
    std::cout << "original color: " << orgColor << std::endl;

    SvgBuilder::ConvertedColor convertedColor;
    convertedColor.originalColorSpace = mode;
    convertedColor.originalColorValue = orgColor;
    ...
    if (!icc.empty()) {
        ...
        convertedColor.rgb = icc_color.str();
        return convertedColor;
    }

    convertedColor.rgb = rgb_color;
    return convertedColor;
}
...
void SvgBuilder::_setStrokeStyle(SPCSSAttr *css, GfxState *state) {
    ...
    if (space->getMode() == csPattern) {
        ...
        sp_repr_css_set_property(css, "stroke", convertGfxColor(state->getStrokeColor(), space).rgb.c_str());
    }
 ...
 * Uses the given SPCSSAttr for storing the style properties.
 */
void SvgBuilder::_setFillStyle(Inkscape::XML::Node *node, SPCSSAttr *css, GfxState *state, bool even_odd) {
    ...
    if (space->getMode() == csPattern) {
        ...
        auto convertedColor = convertGfxColor(state->getFillColor(), space);
        sp_repr_css_set_property(css, "fill", convertedColor.rgb.c_str());
        node->setAttributeOrRemoveIfEmpty("sourceColorSpace", convertedColor.originalColorSpace);
        node->setAttributeOrRemoveIfEmpty("sourceColorValue", convertedColor.originalColorValue);
    }
 ...
 */
SPCSSAttr *SvgBuilder::_setStyle(Inkscape::XML::Node *node, GfxState *state, bool fill, bool stroke, bool even_odd) {
    SPCSSAttr *css = sp_repr_css_attr_new();
    if (fill) {
        _setFillStyle(node, css, state, even_odd);
    } else {
...
bool SvgBuilder::mergePath(GfxState *state, bool is_fill, const std::string &path, bool even_odd)
{
    if (shouldMergePath(is_fill, path)) {
        ...
        SPCSSAttr *css = sp_repr_css_attr_new();
        if (is_fill) {
            _setFillStyle(prev, css, state, even_odd);
...
void SvgBuilder::addPath(GfxState *state, bool fill, bool stroke, bool even_odd) {
    ...
    // Set style
    SPCSSAttr *css = _setStyle(path, state, fill, stroke, even_odd);
                                    ...
                                    double opacity)
{
    ...
    if (space->getMode() == csDeviceGray) {
        ...
        os_opacity << opacity;
        color_text = convertGfxColor(color, space).rgb;
    }
...
void SvgBuilder::_setTextStyle(Inkscape::XML::Node *node, GfxState *state, SPCSSAttr *font_style, Geom::Affine ta)
{
    ...
    auto style = _setStyle(node, state, has_fill, has_stroke);
    state = state->restore();
    if (font_style) {
...
void SvgBuilder::addImageMask(GfxState *state, Stream *str, int width, int height,
    ...
    SPCSSAttr *css = sp_repr_css_attr_new();
    _setFillStyle(rect, css, state, false);
